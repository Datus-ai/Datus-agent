...
class TestReviewerRequest:
    ...
    @pytest.mark.asyncio
    async def test_empty_adapter_error_sentinel_returns_fallback_verdict(self):
        fake_model = MagicMock()
        fake_model.generate_with_json_output.return_value = {"error": "", "raw_response": ""}
        reviewer = LLMAutoReviewer(agent_config=MagicMock())
        request = AutoReviewRequest("bash", {"command": "make"}, {}, {})
        ...
        assert result == AutoReviewVerdict(
            ...
            decision="ask",
            confidence=0.0,
            rationale="AI review unavailable: model returned an invalid response",
        )
