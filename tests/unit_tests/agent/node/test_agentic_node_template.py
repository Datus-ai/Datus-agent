...
# ---------------------------------------------------------------------------
# Which template the system prompt comes from
# ---------------------------------------------------------------------------


class _TemplateNameNode(AgenticNode):
    """Bare node that only exercises ``_get_system_prompt``'s name resolution."""
    ...
    def get_node_name(self) -> str:
        return "jeff_shop_ask"

    def _resolve_workspace_root(self):
        return "/tmp"

    def _finalize_system_prompt(self, base_prompt: str, memory_node_name_override=None) -> str:
        return base_prompt
...
class _CapturingPromptManager:
    ...
    def render_template(self, template_name: str, version=None, **kwargs) -> str:
        self.template_name = template_name
        return "rendered"


class TestSystemPromptTemplateName:
    """``system_prompt`` names the template; the node name is only the fallback.

    A host that names its nodes after the user's sub-agent (SaaS does) would
    otherwise only ever resolve a template FILE named after that sub-agent —
    which is why one used to be copied to disk per sub-agent just to hold a
    builtin's content.
    """

    def _render(self, monkeypatch, node_config: dict) -> str:
        pm = _CapturingPromptManager()
        monkeypatch.setattr(
            "datus.agent.node.agentic_node.get_prompt_manager",
            lambda **_kwargs: pm,
        )
        _TemplateNameNode(node_config)._get_system_prompt()
        return pm.template_name

    def test_the_configured_system_prompt_names_the_template(self, monkeypatch):
        assert self._render(monkeypatch, {"system_prompt": "ask_metrics"}) == "ask_metrics_system"

    def test_the_node_name_is_the_fallback(self, monkeypatch):
        """Unchanged for the CLI, which names its nodes after the template."""
        assert self._render(monkeypatch, {}) == "jeff_shop_ask_system"
