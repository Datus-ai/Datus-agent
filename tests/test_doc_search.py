...
def agent_config() -> AgentConfig:
    return load_acceptance_config(namespace="starrocks")
...
class TestSchemas:
    ...
    def test_platform_doc_chunk_different_inputs(self):
        ...
        id1 = PlatformDocChunk.generate_chunk_id("snowflake", "docs/a.md", 0, "v1")
        id2 = PlatformDocChunk.generate_chunk_id("snowflake", "docs/b.md", 0, "v1")
        id3 = PlatformDocChunk.generate_chunk_id("snowflake", "docs/a.md", 1, "v1")
...
class TestSemanticChunker:
    ...
    def test_chunk_simple_document(self):
        ...
        assert all(c.platform == "test" for c in chunks)
        # Verify new fields
        for chunk in chunks:
...
class TestSearchTool:
    ...
    def setup_store(self, agent_config: AgentConfig):
        ...
        self.store = document_store(agent_config.rag_storage_path())
    ...
    @pytest.mark.parametrize("platform,version", [("polaris", ""), ("polaris", "1.2.0")])
    def test_list_document_nav_returns_tree(self, platform: str, version: str):
        ...
        # Each top-level item should be a tree node with name, children, docs
        for item in result.nav_tree:
            ...
            if "name" in item:
                ...
                assert "docs" in item, "Tree node should have 'docs'"
                assert item["name"], "name should not be empty"
                # docs should be a list of title strings (lightweight for LLM browsing)
                for doc in item["docs"]:
                    assert isinstance(doc, str), f"doc leaf should be a string, got {type(doc)}"
            else:
    ...
    @pytest.mark.parametrize("platform,version", [("polaris", "")])
    def test_get_document_by_title_from_nav(self, platform: str, version: str):
        ...
        # Find a doc leaf from the tree (docs are title strings now)
        # Handles both single-version (name/children/docs) and multi-version (version/tree) formats
        def _find_first_doc(nodes):
            for node in nodes:
                ...
                if node.get("docs"):
                    return node["docs"][0]
                found = _find_first_doc(node.get("children", []))
                if found:
                    return found
            return None
    ...
    @pytest.mark.parametrize("platform,version,keywords", [("polaris", "", ["catalogs", "namespace"])])
...
class TestIntegration:
    ...
    def test_web_fetcher_integration(self):
        ...
        for chunk in all_chunks:
            ...
            assert chunk.chunk_text, "chunk_text should not be empty"
            assert chunk.platform == "snowflake"
