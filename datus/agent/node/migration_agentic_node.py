...
"""
MigrationAgenticNode implementation for cross-database migration.

This module provides a specialized implementation of AgenticNode focused on
migrating data across database engines (e.g., DuckDB to Greenplum/StarRocks).
It includes DB tools with DDL, DML, and cross-database transfer capabilities,
type mapping awareness, and post-migration reconciliation.
"""
...
class MigrationAgenticNode(AgenticNode):
    """
    Cross-database migration agentic node.

    This node provides migration capabilities with:
    - Database tools for schema exploration across multiple databases
    - DDL execution tool for target table creation
    - DML execution tool for data writes
    - Cross-database transfer tool (transfer_query_result)
    ...
    def __init__(
        ...
        super().__init__(
            ...
            description=f"Cross-database migration node: {self.NODE_NAME}",
    ...
    def _setup_db_tools(self):
        """Setup database tools including DDL, DML, and cross-database transfer."""
        try:
            ...
            logger.debug("Added database tools + execute_ddl + execute_write + transfer_query_result from DBFuncTool")
    ...
    async def execute_stream(
        ...
        try:
            ...
            final_action = ActionHistory.create_action(
                ...
                action_type="migration_response",
