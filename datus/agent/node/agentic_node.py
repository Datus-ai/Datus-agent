...
class AgenticNode(Node):
    ...
    def _setup_permission_manager(self) -> None:
        ...
        ``execution_mode="workflow"`` nodes (``/bootstrap``, ``auto_create``,
        etc.) ignore the user's profile and run under a fresh
    ...
    def _merge_skill_patterns(existing_skills: Any, injected_skills: List[str]) -> str:
        ...
        Platform-aware subagents (``gen_dashboard``, etc.) need to
