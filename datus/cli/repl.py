...
class DatusCLI:
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                # Handle case where prompt returns None (e.g., after Shift+Tab exit)
    ...
    def _execute_plan_chat_command(self, message: str):
        """Execute chat command in Plan Mode - unified agentic loop with hook interception"""
        try:
            database_name = self.cli_context.current_db_name or "auto-detected"

            # Use unified flow in single agentic loop
            success = self.plan_manager.execute_unified_plan(
                message=message,
                database_name=database_name,
                chat_executor=self.chat_commands.execute_chat_command_with_hooks,
                user_choice_func=self._get_plan_execution_choice,
            )

            if success:
                self.console.print("[green]✅ Task completed successfully[/]")
                # Ensure todo list is cleared after successful completion
                self.plan_manager.plan_tool.storage.clear_all()
            else:
                self.console.print("[red]❌ Task failed[/]")
    ...
    def _get_plan_execution_choice(self):
        ...
        try:
            ...
            if choice == "1":
                ...
                if feedback:
                    return {"action": "replan", "feedback": feedback}
                else:
    ...
    def _cmd_help(self, args: str = ""):
        ...
        lines.append("[bold]Plan Mode (Shift+Tab to toggle):[/]")
        lines.append(f"    {'[PLAN] mode':<{CMD_WIDTH}}AI creates execution plan before running tasks")
        lines.append(f"    {'[EXEC] mode':<{CMD_WIDTH}}AI executes tasks directly without planning")
        lines.append(f"    {'Plan benefits:':<{CMD_WIDTH}}Review steps, manual confirmation, better control")
        lines.append("")
