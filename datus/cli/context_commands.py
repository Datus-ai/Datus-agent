...
class ContextCommands:
    ...
    def cmd_metrics(self, args: str):
        """Display semantic models and metrics in interactive tree interface."""
        try:
            if not self.cli.agent_config:
                self.console.print("[bold red]Error:[/] No agent configuration.")
                return

            from datus.cli.screen import show_metrics_screen
            ...
            self.console.print(f"[bold red]Error:[/] Failed to display metrics: {str(e)}")
