...
def generate_install_plugins_script(
    ...
    for name in sorted(kept):
        ...
        commands.append(f"datus plugin install {spec}")
    script = "\n".join(
        [
            "#!/usr/bin/env bash",
...
def generate_readme(
    ...
    if has_plugin_script:
        lines.append("bash scripts/install_plugins.sh")
    if env_vars:
        sample = " ".join(f"{binding.var}=..." for binding in _unique_vars(env_vars)[:2])
        lines.append(f"export {sample}   # full list below")
    lines.append("datus-api          # or `datus` for the interactive console")
    lines.append("```")
    if has_rebuild_script:
        lines += [
            "",
            "Then rebuild the metric/semantic search index once:",
            "",
            "```bash",
