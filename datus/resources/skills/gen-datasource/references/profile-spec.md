...
| Lines per document | The row allocation decides the average (1.8 by default, see 1.1), and the spread around it is 1-5 lines with weights 0.52 / 0.26 / 0.12 / 0.06 / 0.04 scaled to that average. A document always has at least one line, so pinning a detail table below its parent is not reachable |
