...
9. **Time dimension**: exactly one primary time field per dataset. When several date columns exist and the primary one is ambiguous, ASK before generating. **Verify `time_granularity` with data, never guess from the column name or comment**: run one query such as `SELECT COUNT(DISTINCT <time_col>), MIN(<time_col>), MAX(<time_col>) FROM <table>` and derive the snapshot interval (e.g. a few month-end dates spanning months → `month`; daily rows → `day`).
