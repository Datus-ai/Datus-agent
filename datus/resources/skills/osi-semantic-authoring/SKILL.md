...
8. **Field selection — decide by role, not by listing every column:**
   ...
   - Use SQL `GROUP BY` evidence and the compact plan's `queryability_contracts[].dimensions` to author the dimensions needed by the requested grouping combinations. A dimension may later be qualified in the plan (for example `activity_info.ac_channel`) after the semantic model exposes its final query name.
...
14. **Model query-backed outputs when needed.** Original request SQL is immutable evidence, but it is not a required dataset definition. When a window, post-aggregation, complex join, or other output cannot be represented cleanly as normal semantic fields and metrics, create or update a query-backed dataset with corrected generated SQL. Mark it with the DATUS query-source extension, expose its final grouping fields as dimensions, and expose metric result columns as plain fields. Record corrected SQL in the compact plan's `generated_sql`; do not expose internal CTE columns that are absent from the final output.
