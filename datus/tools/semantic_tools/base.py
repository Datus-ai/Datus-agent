...
class BaseSemanticAdapter(ABC):
    ...
    def sync_to_storage(self, storage_manager: "SemanticStorageManager") -> Dict[str, int]:  # noqa: F821
        """
        Sync adapter data to unified storage.

        This method is called by the sync mechanism to populate
        SemanticModelStorage and MetricStorage.

        Default implementation syncs both semantic models and metrics.
