...
class ChatService:
    ...
    async def compact_session(
        ...
        try:
            ...
            return Result[CompactSessionData](
                success=True,
                data=CompactSessionData(session_id=session_id, success=True),
            )
    ...
    def get_history(self, session_id: str, user_id: Optional[str] = None) -> Result[ChatHistoryData]:
        ...
        try:
            ...
            event_id = 0
            logger.debug(f"Retrieved {len(raw_messages)} messages for session {session_id}")
            ...
            logger.debug(f"Retrieved {len(sse_messages)} messages for session {session_id}")
