...
def append_enhanced(structured_or_plain: str, additional: str) -> str:
    """Append *additional* text to the ``enhanced`` part of a user prompt.

    If *structured_or_plain* is already a structured JSON message, the
    function appends to the existing ``enhanced`` part (or inserts one
    before the ``user`` part). For a plain string, it builds a new
    structured message ``[enhanced=additional, user=plain]``.

    The original ``user`` part is preserved unchanged so what the human
    sees in the UI is untouched.
    """
    if not additional:
        return structured_or_plain

    if is_structured_content(structured_or_plain):
        try:
            parsed = json.loads(structured_or_plain)
        except (json.JSONDecodeError, TypeError):
            parsed = None
        if isinstance(parsed, list):
            ...
            if enhanced_idx is not None:
                existing = parsed[enhanced_idx].get("content", "") or ""
                joiner = "\n\n" if existing and not existing.endswith("\n") else ""
                parsed[enhanced_idx]["content"] = f"{existing}{joiner}{additional}"
            else:
                parsed.insert(0, {"type": "enhanced", "content": additional})
            return json.dumps(parsed, ensure_ascii=False)

    return build_structured_content(
        [
            MessagePart(type="enhanced", content=additional),
            MessagePart(type="user", content=structured_or_plain),
        ]
    )
