...
class ReferenceSqlStorage(BaseEmbeddingStore):
    ...
    def create_indices(self):
        ...
        self.create_fts_index(["sql", "name", "comment", "summary", "search_text", "tags"])
...
class ReferenceSqlRAG:
    ...
    def search_reference_sql(
        ...
        """Search reference SQL by search text using vector search.

        Args:
            query_text: Query text for vector search
            domain: Optional domain filter
            layer1: Optional layer1 filter
            layer2: Optional layer2 filter
            top_n: Number of top results to return
            selected_fields: Optional list of fields to return. If None, returns all default fields.
        ...
        if search_results:
            ...
            logger.debug(f"No reference SQL results found for query: {query_text}")
            return []
    ...
    def get_reference_sql_detail(self, domain: str, layer1: str, layer2: str, name: str) -> List[Dict[str, Any]]:
        return self.reference_sql_storage._search_all(
            ...
            ["name", "summary", "comment", "search_text", "tags", "sql"],
        ).to_pylist()
