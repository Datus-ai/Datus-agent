#!/bin/sh
# install.sh - one-liner installer for Datus-Agent on Linux and macOS.
#
# Usage:
...
#   curl -fsSL ... | DATUS_VERSION=0.2.6 sh
# ):
#   DATUS_VERSION          Pin a PyPI version (e.g. 0.2.6). Ignored with DATUS_FROM_GIT=1.
#   DATUS_FROM_GIT=1       Install from GitHub source instead of PyPI.
#   DATUS_REF              Git ref (branch/tag/commit) for DATUS_FROM_GIT. Default: main.
...
validate_env() {
    if [ -n "$DATUS_FROM_GIT" ] && [ -n "$DATUS_VERSION" ]; then
        die "DATUS_FROM_GIT and DATUS_VERSION are mutually exclusive"
    fi
...
# Make sure a freshly installed uv is on PATH for the current script run.
_extend_path_for_uv() {
...
install_package() {
    if [ -n "$DATUS_FROM_GIT" ]; then
        need_cmd git
        spec="git+${GIT_REPO}@${DATUS_REF}"
        info "installing $PYPI_PACKAGE from GitHub source: $spec"
    elif [ -n "$DATUS_VERSION" ]; then
        spec="${PYPI_PACKAGE}==${DATUS_VERSION}"
        info "installing $spec from PyPI"
    else
        spec="$PYPI_PACKAGE"
        info "installing latest $PYPI_PACKAGE from PyPI"
    fi
...
write_shims() {
    ...
    for name in $CONSOLE_SCRIPTS datus-pip; do
        ...
        cat >"$target" <<EOF
#!/bin/sh
# generated by datus-agent install.sh; regenerated on each install
exec "$venv_target" "\$@"
EOF
        chmod +x "$target"
    done
...
path_contains_bin_dir() {
    # Check the user's original PATH, not the one we mutated to find uv.
...
${GREEN}${BOLD}Datus-Agent installed.${RESET}
