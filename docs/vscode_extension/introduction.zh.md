...
1. 打开活动栏的 **Datus Studio**，点击右上角齿轮图标进入 **Datus · Settings**。
...
- **保持 VSCode 工作区干净**：因为读写都落在工作区根，建议每个 Datus 项目对应一个 VSCode 工作区文件夹，与 Datus-agent 的 `project_name`、`subject/` 目录一一对应，子代理生成的 `metric.yml`、`semantic_model.yml`、`reference_sql/*.sql` 才会落到正确位置。
