...
- **Grouping recommendation is separate from the OSI field shape.** A reachable field may appear in the native metric-scoped `get_dimensions` catalog whether or not it has a `dimension:` block. D-DIM `is_dimension` explicitly controls `recommended`; when it is omitted, Dosi infers the recommendation from time, key, relationship, and measure roles. `recommended=false` is a selection hint, not a group-by prohibition. Field-level `type` hints are not part of the authoring contract.
- **Keys come only from source DDL.** Transcribe declared physical primary keys into `primary_key` and declared unique constraints or whole-table unique indexes on plain columns into `unique_keys`, preserving complete composite keys and their column order. JOIN patterns and profiling do not establish keys, and semantic modeling does not scan table data to discover or verify them. If DDL provides no usable key, omit that key and dependent new relationships, then continue the remaining modeling work.
