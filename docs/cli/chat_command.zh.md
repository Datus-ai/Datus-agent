...
按 **Ctrl+P** 可按 **normal → auto → dangerous → normal** 的顺序循环切换权限模式。智能体运行过程中该快捷键仍然有效，新模式会应用于当前对话后续的工具权限检查。

### 运行中插入消息

智能体运行过程中，你不必等它结束才能补充上下文。在输入框中输入文本并按 **Enter**，文本会被排入待发送队列，并在下一次 LLM turn 之前发送给模型——它会影响当前这一轮运行，而不是开启一段新对话。

工作机制：

- 底部状态栏上方会出现**置顶预览**，列出队列中所有待注入的消息。队列被消费完时，预览自动折叠为零行。
- 每次 LLM turn 之前，队列中所有排队消息都会被 drain，作为 `user` 项追加到模型输入并持久化到会话历史。
- 如果运行已进入最终 turn 但队列仍非空，残余消息会**自动续跑**到一段新的运行中，避免你输入的内容被静默丢弃。
- 插入的消息会在 `Ctrl+O` verbose/compact 切换以及 `/resume` 之后保留在转录中，让你始终能看到自己注入的内容。

此能力与 `ESC` / `Ctrl+C`（打断）以及 `/chat/stop` 是**互补**而非替代关系。需要用新上下文**引导**当前运行时用插入；需要**中止**当前运行时用打断。
...
- `/compact`：压缩历史对话，减少上下文占用同时保留关键信息
  ...
  - 自动触发：当模型上下文使用率超过 90%（可配置）时也会自动执行 major compact；此外随着会话变长，更轻量的 **minor compact** 会在每次模型请求前的统一检查中归档符合条件的旧工具输出，第一次请求也会检查
