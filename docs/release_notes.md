...
- **REST API, IM Gateway & CLI Docs** - New docs for REST API deployment / chat / KB / models, Slack & Feishu IM gateways, and `/model` / `/language` / `/effort` / `/init` / `/bootstrap` / service / `--datasource` flows. [docs](https://docs.datus.ai/API/deployment/)
- **Configuration Docs** - Added datasources, semantic layer, BI platforms, schedulers, and PostgreSQL-backed storage configuration docs. [docs](https://docs.datus.ai/configuration/datasources/)
- **Subagent Docs** - Dashboard generation, table generation, scheduler workflow, data pipeline, metrics, semantic model, and SQL summary subagent docs. [docs](https://docs.datus.ai/subagent/builtin_subagents/)
