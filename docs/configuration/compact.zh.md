...
**触发条件** —— 当会话的 user 轮次超过 `keep_recent_user_turns`（默认 4）时，minor compact 进入可触发状态，并在工具调用之后运行。它和 major 一样是同步阻塞的，但由于它只是纯本地的、基于规则的归档（无 LLM 调用），很快就能完成，几乎不会拖慢 agent 发起下一次模型调用。
