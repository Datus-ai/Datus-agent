...
"""
Save generated external knowledge YAML file to the knowledge base.
...
def main():
    parser = argparse.ArgumentParser(description="Save ext knowledge to knowledge base")
    parser.add_argument("--file-path", type=str, required=True, help="Ext knowledge YAML file name")
    parser.add_argument(
    ...
    ext_knowledge_dir = path_manager.ext_knowledge_path(agent_config.current_namespace)
    full_path = os.path.realpath(str(ext_knowledge_dir / args.file_path))

    if not full_path.startswith(os.path.realpath(str(ext_knowledge_dir)) + os.sep):
    ...
    try:
        ...
        result = GenerationHooks._sync_ext_knowledge_to_db(full_path, agent_config, args.build_mode)
